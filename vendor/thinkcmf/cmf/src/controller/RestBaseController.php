...
class RestBaseController
{
    ...
    private function _initWechatUser()
    {
        ...
        $user = WechatUserModel::where('openid', $openid)->find();

        if (!empty($user)) {
            $this->wechatUserId = $user['id'];
        }
